...
pub const SOLVER: Solver = Solver {
    ...
    solve_2: |input| {
        ...
        // through my puzzle input step-by-step and mapping out the logic, I determined that the
        // initial values in the b and c registers define an inclusive lower and upper bound for a
        // range of values, which are offset from each other by a hardcoded value in the
        // "sub b -<SOME VALUE>" instruction on the second-last line (e.g. if b is 4, c is 10, and
        // the hardcoded value is 2, the values that the program would iterate over are be 4, 6, 8,
        // 10). For each of these values, it sets register f to 1, then it creates a sub-loop using
        // register d, which itself has its own sub-loop using register e, both of which count from
        // 2 to the current value. In every iteration of the innermost loop, it checks if
        // d*e - b == 0, and if this is true, it sets f to 0. Once the outer sub-loop completes, if
        // f was set to 0, register h is incremented by 1.
        //
        // The condition d*e = b == 0 is equivalent to d*e == b. If this is never true, then
        // register f remains set to 1 and h isn't incremented; if it is true at least one time,
        // then register f is set to 0 and h gets incremented. As such, for each value, the program
        // essentially asks "Does there exist any two integers which are both greater than or equal
        // to 2, such that their product equals the given value?" This is equivalent to asking if
        // the given value is a composite number (a positive integer other than 1 which is not a
        // prime number). As such, what the program ultimately does is count every composite number
        // within the set of numbers defined by the bounds of registers b and c and the hardcoded
        // offset, and stores the count in register h.
        //
        // I assume this is the intended trick of the puzzle, and that every puzzle input is set up
        // to follow the rules described above. As such, my solution searches the program in the
        // puzzle input for every line that modifies registers b or c, executes all of them except
        // for the last one, assumes the values for b and c I end up with are the initial values for
        // them, and assumes the literal argument to the last instruction instead defines the
        // offset. I ignore all other lines in the puzzle input, assuming that the rest of it is
        // just implementation details for counting composite numbers. I use the extracted b, c, and
        // offset values to set up the full list of values, then use my own implementation for
        // counting composite numbers. If any of the assumptions I have made for this solution are
        // incorrect, then my solution might not work for all possible puzzle inputs.

        fn is_prime(x: u32) -> bool {
            // Technically, an is_prime() function would need to check if the input is 1 or lower
            // and immediately return false if so, but it's known this function will never be ran
            // for a value smaller than 2 within this solver, so this check is omitted.

            // When checking for prime numbers, only integers up to the square root of the number
            // need to be checked (any factor greater than the square root will need to be
            // multiplied by a factor smaller than the square root to equal the given number, so if
            // the number isn't prime, it will always have at least one factor (other than 1) that
            // is smaller than its square root).
        ...
        fn is_writing_to_b_or_c(instruction: &Instruction) -> bool {
            match instruction {
                Instruction::Set(op_1, _)
                | Instruction::Sub(op_1, _)
                | Instruction::Mul(op_1, _) => {
                ...
                // The jnz function doesn't modify any registers so always return false (in
                // principle they might affect b and c through looping around another instruction
                // that affects it, however since only the initial values of b and c are needed,
                // it's assumed that once jumping is done to begin looping, the initial values of b
                // and c have been finalized and this jnz functions can still be safely ignored).
        ...
        let offset = if let Instruction::Sub(_, op_2) = last_instruction {
            if let RegisterOrValue::Value(value) = op_2 {
                // value is expected to be negative as it's used in a sub instruction with the
                // intention of adding an amount to register b, so the absolute value of it must be
                // taken.
                value.unsigned_abs()
            } else {
