...
pub const SOLVER: Solver = Solver {
    ...
    solve_2: |input| {
        ...
        // booleans that's true for digits that are 1 (representing used squares) and false for
        // digits that are 0 (representing free squares).
        ...
        // least significant digit, which can be tested to determine its value. Then a right shift
        // is performed to move the next digit into the least significant bit position and the
        // process repeats for all 128 binary digits. Note that assigning the booleans this way
        // flips the grid horizontally, but that's fine because this doesn't affect the connectivity
        ...
        for (y, knot_hash_bool_row) in knot_hash_bool_array.iter().enumerate() {
            for (x, square) in knot_hash_bool_row.iter().enumerate() {
                // Check if the current square is used and is not part of any previously-found
                // group.
                if *square && !visited_points.contains(&Point { x, y }) {
                    // Increment groups, then find all squares connected to this square and record
                    // them so they aren't double-counted.
                    groups += 1;
