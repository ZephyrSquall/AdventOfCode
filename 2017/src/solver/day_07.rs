    ...
    solve_2: |input| {
        ...
        impl Disc {
            ...
            fn build_disc(input: &str, program: &str) -> Disc {
                ...
                // If no value is returned from the above loop, then the program name wasn't found
                // in any of the lines of the input. This can only happen if the input is malformed
                // or this solver has a logic error. Either way, no valid Disc can be returned, so
                // panic.
        ...
        fn fix_wrong_weight(correct_cumulative_weight: usize, disc: Disc) -> usize {
            ...
            for sub_tower in disc.sub_towers {
                ...
                {
                    // Check if all sub-towers have equal cumulative weights. If so, the current
                    // disc must be the incorrect disc (as there is exactly one incorrect disc, none
                    // of the sub-towers can be incorrect, and this function is only called on discs
                    // with incorrect cumulative weights). It's assumed that no disk has exactly one
                    // sub-tower, as then the puzzle would have an ambiguous answer as it would be
                    // impossible to tell whether the incorrect value lies with that disc or its
                    // single sub-tower. Unlike with the very bottom disc, tracking the correct
                    // cumulative weight makes it possible to identify which sub-tower has the wrong
                    // weight when there's exactly two of them.
                    let first_sub_tower_cumulative_weight =
                        sub_tower.sub_towers[0].cumulative_weight;
                    ...
                    if are_sub_towers_equal {
                        // This sub-tower has been identified as the disc with the incorrect weight,
                        // so calculate what its weight should be.
                        //
                        // Let the weight be w, cumulative weight be c, correct weight be wc,
                        // correct cumulative weight be cc, and the sum of weights of all sub-towers
                        // be s. The cumulative weight is obtained by adding the disc's weight to
                        // the weight of all sub-towers, so:
                        // c = w + s
                        // cc = wc + s
                        //
                        ...
                        // s = cc - wc
                        //
                        // As these are both equal to s, they can be equated and rearranged in terms
                        // of wc:
                        // c - w = cc - wc
                        // wc = w + cc - c
                        //
                        // The correct weight wc is the solution to the puzzle, so calculate it
                        // using this equation and return it.
                    ...
                    // exactly one incorrect weight, it must lie with that different sub-tower or
                    // one of its sub-towers, so call this function recursively on it. Return the
                    // value from the recursive function call to propagate the return value back up
                    // to the original caller.
                    //
                    // As this own sub-tower's weight has been verified to be correct, it can be
                    // safely subtracted from the correct cumulative weight so far to get the
                    // correct cumulative weight of all sub-towers, and then divided by the number
                    // of sub-towers to get the correct cumulative weight per sub-tower.
            ...
            // If no value is returned from the above loop, then then no sub-tower was found to have
            // an erroneous weight. This can only happen if the input is malformed or this solver
            // has a logic error. Either way, no valid correct weight can be returned, so panic.
            panic!()
        }
