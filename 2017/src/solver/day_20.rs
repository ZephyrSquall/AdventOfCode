...
pub const SOLVER: Solver = Solver {
    ...
    solve_2: |input| {
        // This vector refers to a Euclidean vector (a value with a magnitude and direction), not
        // the type of a collection in Rust named Vec. It's postfixed with "3d" to make this
        ...
        // I'm assuming that no further collisions occur after 100 ticks. I have run the simulation
        // up to 1 million ticks and found no further collisions occur after the 40th tick. To solve
        // this with absolute certainty would most likely require very complicated analysis around
        // finding the line that each particle follows in the long term, making a buffer around that
        // line to account for never perfectly following that line in a finite amount of time, and
        // checking that none of these buffers overlap. This feels like a level of complexity that
        // is outside the scope of an AoC puzzle.
        for _ in 0..100 {
            ...
            for (particle_0, particle_1) in particles.iter().tuple_combinations() {
                if particle_0.position == particle_1.position {
                    // The position must be cloned instead of borrowed in case this particle is
                    // removed before other particles that collided at this position are checked (a
                    // mutable borrow of an immutable reference would occur in the retain statement
                    // if this were not cloned).
