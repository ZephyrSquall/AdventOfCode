...
pub fn solve_part_1() -> i32 {
    ...
    //     maximum Manhattan Distance of that loop (directly right of the 1), then steps up until it
    //     reaches the maximum value (the upper-right corner of the loop), then repeats this process
    //     three more times to traverse the other three sides of the loop.
    ...
    //     the minimum Manhattan Distance of a loop (which is half of the maximum) is always a whole
    //     number.
...
pub fn solve_part_2() -> i32 {
    ...
    //     loop. (The first and second 1s in the data don't fall neatly into any of these four cases
    //     and technically form a fifth and sixth case (zero adjacent numbers and one adjacent
    //     number). Because no other numbers share these cases, it is simpler to just skip these
    //     cases by hardcoding the first and second numbers.)
    ...
    //         the immediately preceding value, the value at the index, the value immediately before
    //         that index, and the value immediately after that index. Increment the index after
    //         this step.
    //      - Immediately before a corner: The adjacent squares are the immediately preceding value,
    //         the value at the index, and the value immediately before the index. Do not increment
    //         the index after this step.
    //      - On a corner: The adjacent squares are the immediately preceding value and the value at
    //         the index (which is diagonally adjacent instead of orthogonally adjacent because the
    //         index wasn't incremented last step). Do not increment the index after this step.
    ...
    //     lasts four edges and the edge length increases by two after each two, however the way one
    //     loop leads into the next causes the last edge of the inner loop to be one step longer
    //     than normal and the first edge of the outer loop to be one step shorter than normal.
