...
pub struct Solver<'a> {
    pub day: u8,
    pub title: &'a str,
    pub solve_part_1: fn(input: &str) -> Solution,
    pub solve_part_2: fn(input: &str) -> Solution,
}

// Solution variants that are not currently used in any solvers are commented out to avoid dead_code
// warning.
#[derive(Debug)]
pub enum Solution {
    // U8(u8),
